...
public Action:TF2Items_OnGiveNamedItem(client, String:classname[], iItemDefinitionIndex, &Handle:item)
{
	...
	//TODO: Support comma-delimited strings, eg "38, 457" or "tf_weapon_knife, tf_weapon_katana"
	...
	else
	{
		...
		if(boss!=-1)
		{
			if(attacker<=MaxClients)
			{
				...
				KvRewind(kvWeaponMods);
				if(KvJumpToKey(kvWeaponMods, classname) || KvJumpToKey(kvWeaponMods, indexString))
				{
					Debug("Entered classname %s or index %i", classname, index);
					if(KvJumpToKey(kvWeaponMods, "onhit"))
					{
						...
						for(new key; KvGoToNextKey(kvWeaponMods); key+=2)
						{
							...
							Debug("\t\tFound action set %s ; %i", action, value);
							if(StrEqual(action, "glow on scoped hit", false))
							{
								if(CheckRoundState()!=FF2RoundState_RoundEnd)
								{
									new Float:charge=(IsValidEntity(weapon) && weapon>MaxClients ? GetEntPropFloat(weapon, Prop_Send, "m_flChargedDamage") : 0.0);
									if(index==752)  //Hitman's Heatmaker  FIXME: THIS IS STILL HARDCODED
									{
									...
									else if(index!=230 && index!=402 && index!=526)  //Sydney Sleeper, Bazaar Bargain, Machina  FIXME: THIS IS STILL HARDCODED
									{
									...
									{
										if(index!=230 || BossCharge[boss][0]>90.0)  //Sydney Sleeper  FIXME: THIS IS STILL HARDCODED
										{
											damage*=3.0;
										}
										else
							...
							else if(StrEqual(action, "modify health"))
							{
								...
								new newhealth=health+value;
								if(health<max+100)  //FIXME: THIS IS STILL HARDCODED
								{
								...
								for(new healer; healer<healerCount; healer++)
								{
									if(IsValidClient(healers[healer]) && IsPlayerAlive(healers[healer]))
									{
										...
										if(IsValidEntity(medigun))
										{
											...
											{
												new Float:uber=GetEntPropFloat(medigun, Prop_Send, "m_flChargeLevel")+(value/healerCount);
												new Float:max=1.0;
