...
{
	LogMessage("Freak Fortress 2 v%s Loading...", PLUGIN_VERSION);
...
public Action OnPlayerRunCmd(int client, int &buttons, int &impulse, float vel[3], float angles[3], int &weapon)
{
	...
	if(IsValidEntity(entity) && IsValidEntity(weapon) && GetClientTeam(client)==OtherTeam && SapperCooldown[client]<=0)
	{
...
stock int ParseFormula(int boss, const char[] key, const char[] defaultFormula, int defaultValue)
{
	...
	float result=EvaluateFormula(formula);
	if(result<=0.0)
	{
