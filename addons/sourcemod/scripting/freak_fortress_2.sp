...
public OnPluginStart()
{
	...
	cvarUseCountdown=CreateConVar("ff2_countdown", "120", "Amount of time until to wait for until the round ends in a stalemate (begins when it becomes 1v1)", FCVAR_PLUGIN);
	...
	cvarEnableEurekaEffect=CreateConVar("ff2_enable_eureka", "0", "0-Disable the Eureka Effect, 1-Enable it", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	...
	cvarHalloween=CreateConVar("ff2_halloween", "0", "0-Disable Halloween mode, 1-Enable Halloween mode, 2-Use TF2 logic (tf_forced_holiday 2)", FCVAR_PLUGIN, true, 0.0, true, 2.0);
...
public AddToDownload()
{
	...
	if(!FileExists(s))
	{
		if(halloween)
		{
			LogError("[FF2] Freak Fortress 2 disabled-can not find characters_halloween.cfg!");
		}
		else
		{
...
public Action:event_round_end(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	for(new client=0; client<=MaxClients; client++)
	{
		...
		for(new number=0; number<=1; number++)
		{
			if(BossInfoTimer[client][number]!=INVALID_HANDLE)
			{
				KillTimer(BossInfoTimer[client][number]);
				BossInfoTimer[client][number]=INVALID_HANDLE;
			}
		}
	}
	...
	if(BossIsAlive)
	{
		...
		if(RandomSound("sound_fail", s, PLATFORM_MAX_PATH, BossIsAlive))
		{
			EmitSoundToAll(s);
			EmitSoundToAll(s);
		}
...
public Action:MakeBoss(Handle:hTimer,any:index)
{
	...
	{
		if(CheckRoundState()==0) TF2_RespawnPlayer(Boss[index]);
				...
				switch(wepindex)
				{
					...
					case 14, 201, 230, 402, 526, 664, 752, 792, 801, 851, 881, 890, 899, 908, 957, 966:
					{
						...
						if(!(damagetype & DMG_CRIT))
						{
							...
							{
								if(wepindex!=230 || BossCharge[index][0]>90) damage*=2.9;
								else damage*=2.4;
							}
							return Plugin_Changed;
						}
